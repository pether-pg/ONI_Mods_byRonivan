...
namespace MoreCuisineVariety
{
    public class Food_FishWrap : IEntityConfig
    {
        ...
        static Food_FishWrap()
        {
            string[] textArray1 = new string[] { "A tasty snack made by wrapping grilled ", UI.FormatAsLink("Pacu Fillet", "FishMeat"), " with ", UI.FormatAsLink("Warm Flat Bread", "FlatBread"), ". Each bite leaves a mild warm sensation in one's mouth, even when the snack itself is served cold." };
            Description = string.Concat(textArray1);
            RecipeDescription = "Bake a " + UI.FormatAsLink("Pacu Wrap", "FishWrap");
        }
        ...
        {
            GameObject obj3 = EntityTemplates.ExtendEntityToFood(EntityTemplates.CreateLooseEntity("FishWrap", Name, Description, 1f, false, Assets.GetAnim("food_fish_wrap_kanim"), "object", Grid.SceneLayer.Front, EntityTemplates.CollisionShape.RECTANGLE, 0.8f, 0.4f, true, 0, SimHashes.Creature, null), new EdiblesManager.FoodInfo("FishWrap", "", 2200000f, 2, 255.15f, 277.15f, 2400f, true));
            ComplexRecipe.RecipeElement[] ingredients = new ComplexRecipe.RecipeElement[] { new ComplexRecipe.RecipeElement("FlatBread", 1f), new ComplexRecipe.RecipeElement("FishMeat", 1f) };
            ComplexRecipe.RecipeElement[] results = new ComplexRecipe.RecipeElement[] { new ComplexRecipe.RecipeElement("FishWrap", 1f) };
